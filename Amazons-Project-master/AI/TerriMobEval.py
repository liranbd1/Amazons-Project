...
class TerriMobEval:
    ...
    def nearestQueen(self, row, col):
        ...
        white_found = False
        queen_found = False
        move_list.remove([row, col])
        square_moves = deque([])  # make queue
        square_moves.extend(self.addSquareMoves(row, col, move_list))  # finding all the the legal moves from this square
        while queen_found is False:
            ...
            while len(square_moves) != 0:
                ...
                if len(square_moves) == 1 and black_found and not white_found:
                    queen_found = True
                ...
                if len(square_moves) == 1 and white_found and not black_found:
                    queen_found = True
                ...
                if self.board[move[0]][move[1]] == EMPTY_SPACE:
                    temp_queue.extend(self.addSquareMoves(move[0], move[1], move_list))
    ...
    def addSquareMoves(self, curRow, curCol, move_list):
        moves = []
        for square in move_list:
            if IsMoveLegal(square, [curRow, curCol], self.size, self.board):
                moves.append(square)

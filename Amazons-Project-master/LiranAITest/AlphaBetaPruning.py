...
from AI.TerriMobEval import TerriMobEval
...
def sort_moves(moves_to_sort, player):
    ...
    for move in moves_to_sort:
        ...
        value = soft_evaluate()
        max_moves[value] = move   # Adding for dict
        iteration += 1   # iteration is for later upgrades
        undo_move_to_board(move, player)
    sorted_max_moves = collections.OrderedDict(sorted(max_moves.items()))
    print("Sorting Done")
    return list(sorted_max_moves.values())

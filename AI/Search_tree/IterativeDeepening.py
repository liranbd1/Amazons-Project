...
def aspiration(starting_board_matrix, max_depth, previous, size, player_queens, enemy_q, turn_number):
    alpha = previous - WINDOW_SIZE
    beta = previous + WINDOW_SIZE
    result, move = start_alpha_beta(starting_board_matrix, max_depth, size, player_queens, enemy_q,
                                    turn_number, alpha, beta)
    if move is False:
        alpha = -MAX
        beta = MAX
        result, move = start_alpha_beta(starting_board_matrix, max_depth, size, player_queens, enemy_q, turn_number,
                                        alpha, beta)
        return result, move
    else:
        return result, move
